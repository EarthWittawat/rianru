...
8. Optional, and worth it. Build the learning path and the retrieval indexes:

   ```sh
   python scripts/set_positions.py --class ABC123     # the teaching order
   python scripts/build_path.py --class ABC123 --refine  # prerequisites
   python scripts/build_colbert.py --class ABC123     # late-interaction text search
   python scripts/build_colpali.py --class ABC123     # page-image search, needs a GPU
   ```
...
| Page | What it is for |
|---|---|
| `/viewer` | Pick a document, read it, select text to explain, save highlights. Also searches pages by what they look like |
...
- [x] Late-interaction text retrieval (ColBERT), with dense as the fallback
- [ ] Bound the coach's agent loop — a cold plan has taken over ten minutes, with
      no step cap or timeout
- [x] Retrieval over figures, tables and slide layout, through ColPali page-image search
- [ ] Interface redesign, in progress
- [ ] Embedded notebook execution, deferred on purpose
- [x] More than one course, chosen in the interface and scoped through retrieval
